...
| Side effects | Computes `matchPercentage` from a weighted combination of hard filters (must-pass) plus soft factors (`teachingStylePreference`, schedule overlap) — the exact weighting formula is an implementation detail not pinned down in Docs 01–04 beyond "match percentage," and should be documented as a named constant/config once decided, not scattered inline. Creates or reuses a `MatchRequest(status: SEARCHING)` row for tracking. |
...
| Side effects | Ends the current `CohortMembership`-to-tutor link; if a single eligible replacement tutor can take the full remaining group, hands off to `adminMatching.service.ts → manuallyAssignTutor`; otherwise flags for `manuallyAssembleGroup`. |
