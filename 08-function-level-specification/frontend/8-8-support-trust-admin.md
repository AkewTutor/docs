...
| Field | Detail |
|---|---|
| Signature | `useResolveDispute(): UseMutationResult<AdminComplaintDetail, AxiosError, { complaintId: string; status: 'UNDER_REVIEW' \| 'RESOLVED' \| 'DISMISSED'; resolutionAction?: ResolutionAction; resolutionNotes: string; refundAmount?: string }>` |
...
| Behavior | Renders `openDisputes`, `overdueMatchApprovals`, `recordingComplianceEscalations`, `pendingPayoutBatches` as four headline stat cards, plus `generatedAt` as a small "last updated" timestamp — since `usePlatformHealth` polls every 60s (per its query definition above), this timestamp reassures the viewer the numbers are current rather than stale from a much earlier load. |
| ⚠️ Flagged, not silently resolved | `08-support-trust-admin-frontend.md` §8.5 already flags that `TutorPerformanceTable.tsx` (listed in Doc 05b's file inventory for this feature) has no backing data source in `08-support-trust-admin-api.md` — `GET /admin/reports/platform-health` returns only the four aggregate counts above, nothing per-tutor. **This file does not invent a `useTutorPerformance` hook or render `TutorPerformanceTable` on this page** to paper over that gap; `PlatformReportsPage` renders only `PlatformStatsGrid` until one of the two resolutions Doc 07 already named is decided (a new `GET /admin/reports/tutor-performance` endpoint, or sourcing this component from `06-gamification-engagement-api.md`'s `GET /admin/badges` combined with the admin people list from `02-accounts-guardianship-api.md`). Whoever owns Doc 06/07 API should confirm which path before this component is added to any routing tree. |

**States:** loading · success

---

### 8.6 Note on the UC-66/UC-87 Cross-Reference (carried forward from the API spec)

`08-support-trust-admin-api.md`'s own header already flags that Doc 03's UC-66 main flow references "UC-88 in Section Q" where UC-87 (*Admin manages complaints and disputes*) is actually the matching use case. This function-level spec's `DisputeQueuePage`/`DisputeCard` are built against UC-87 as the correct target, consistent with the API file's resolution — this doc does not re-open that question, only notes that the same UC-87 linkage is what `DisputeQueuePage`'s behavior above is written against, in case whoever resolves the Doc 03 typo needs to trace which frontend files assumed which UC number.
