...
> ⚠️ **The guards shown above are defaults, not blanket subtree locks.** `/student/*` → `ProtectedRoute(['STUDENT'])` describes the typical case, not every route under that URL prefix. Several `/student/...` leaf routes are deliberately reachable by Parent too (e.g. `/student/find-tutor`, `/student/upcoming-classes`, `/student/achievements`) — a Parent acting for a Grade 1–5 child performs the same actions the child would, mirroring the API's `Student|Parent` auth label. Each feature file (Docs 01–08 below) declares its own leaf-route guard explicitly, and **that per-route declaration is authoritative** — it may be `['STUDENT','PARENT']` or `'any'` even though the route lives under `/student/`. Do not wrap `/student/*` in a single top-level `<Route element={<ProtectedRoute roles={['STUDENT']} />}>` that every child route inherits from; each leaf route gets its own `ProtectedRoute` per its owning feature file, exactly as specified there.
