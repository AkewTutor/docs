...
| Resend has no built-in cooldown of its own | mock `POST /auth/resend-verification` | call `mutate(userId)` twice in immediate succession | **both calls actually fire** — the hook itself does not throttle; throttling is `VerifyContactPage`'s own 30s UI-disable, tested separately in §9.6. This case exists specifically to confirm the two layers aren't confused: if this hook silently swallowed a rapid second call, `VerifyContactPage`'s own throttle test would give a false sense that a real control exists when it's only a UI courtesy (per 8-1's explicit note that the backend enforces no rate limit here — OWASP A04:2021 Insecure Design / brute-force-adjacent gap, flagged again in §9.10) |
...
- **Client-side resend-verification throttle as a security control** — explicitly reasserted here per Doc 8-1: this 30-second UI disable in `VerifyContactPage` is tested (§9.6) as a UX courtesy only. It provides **no actual rate-limiting** — a person can trivially call the underlying endpoint directly (devtools, a script, a second tab) to bypass it entirely, and §9.5's `useResendVerification` test deliberately proves the hook itself has no throttle. This mirrors the backend test doc's own flagged gap (`9-1-shared-config.md` §9.19): no server-side rate limit is documented anywhere in Docs 02/06/08 for resend-verification, forgot-password, or login. This frontend doc does not fabricate a security test for a control that doesn't exist; it instead makes the absence explicit so nobody mistakes the passing UI test for a mitigation (OWASP A04:2021 – Insecure Design / brute-force exposure).
