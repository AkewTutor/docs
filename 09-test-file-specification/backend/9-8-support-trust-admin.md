...
| REFUND_ISSUED calls the exact same sessions-delivered proration path as any other refund reason (H4 fix) — **I1 fix** | mock `affectedCohortMembershipId` has an active, paid billing cycle | call `resolveDispute(complaintId, adminId, { status: 'RESOLVED', resolutionAction: 'REFUND_ISSUED', affectedCohortMembershipId, resolutionNotes: '...' })` | spy on `refund.service.createPendingRefund` — called with `(payment.id, 'ADMIN_DISPUTE_RESOLUTION')` (which internally invokes `calculateProration`), then immediately `approveRefund(refund.id, adminId)` — confirms this is never a bare client-supplied amount, the exact H4-fixed integrity guarantee, and that the resulting `Refund` ends at `status: APPROVED` rather than being left `PENDING` |
...
- **`FR-SP-044` ("provide account and privacy settings management")** — **explicitly flagged as an unspecified surface, not a test-doc omission**. No service, endpoint, or hook in Doc 08 (backend or frontend) currently owns this requirement — it is not folded into `studentProfile.service.ts` (which covers only `FR-SP-006–010`, academic/basic profile fields) or into any `shared-config` preferences endpoint. This doc cannot write a test file for a surface that hasn't been assigned an implementation home yet. **Action needed from whoever owns Doc 08**: either point `FR-SP-044` at an existing/planned function (e.g. a `User`-level preferences endpoint) so this doc can cite and test it, or confirm it is intentionally deferred past the current build scope.
