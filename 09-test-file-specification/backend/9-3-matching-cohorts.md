...
| Refund amount uses the sessions-delivered proration formula (cross-referenced) | spy on the call into `refund.service.ts` | call `requestSwitch(...)` | assert `refund.service.calculateProration` (or equivalent) is invoked with the current cohort's `totalSessionsBilled` and sessions-delivered-so-far — the actual proration-math assertions live in `9-7-payments-earnings.md`; this test only confirms the correct handoff occurs from this function |
